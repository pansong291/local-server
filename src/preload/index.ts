...
import wrapShutdown, { WithShutdown } from 'http-shutdown'
import { IPAddress, NetFamily, ServerInfo, StartServerConfig } from '@/preload/types'

// borrowed from ry who stole it from jack- thanks
// https://github.com/ry/node_chat/blob/master/fu.js
const mime = {
  // returns MIME type for extension, or fallback, or octet-steam
  lookupExtension: function (ext, fallback?: string) {
 ...
 * 生成控制器
 */
function createController(base: string, cors?: boolean, showDir?: boolean | null) {
  return (req, res) => {
 ...
 * 创建服务器
 */
function createServer(hostname: string, port: number, controller) {
  return new Promise<WithShutdown>((resolve, reject) => {
    const server = wrapShutdown(http.createServer(controller))
 ...
 * 获取 IP 地址
 */
function getIPAddresses(family?: NetFamily, internal?: boolean) {
